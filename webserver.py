...
def handle_one_http_request(conn):
    ...
    cookies = {"name": get_cookie(req.headers, "name"), "favcolor": get_cookie(req.headers, "favcolor")}
...
def handle_http_get_whoami(req, conn):
    ...
    last_active_time = time.strftime("%a, %d %b %Y %H:%M:%S %Z", time.gmtime(conn.last_active_time))
